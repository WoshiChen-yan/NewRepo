...
class WStarter(object):
    ...
    def start(self, **kwargs):
        ...
        wm = subprocess.call(['which', 'wmediumd1'], 
    ...
    def initialize(self, **kwargs):
        ...
        if wmediumd_mode.mode is w_cst.SPECPROB_MODE:
            ...
            if wmediumd_mode.mode is w_cst.SNR_MODE \
                ...
                per_data_file = str(files('mn_wifi') / 'data/signal_table_ieee80211')
                cmdline.append(per_data_file)
        ...
        if self.wmd_process.poll() is not None:
            print("wmediumd is not running")
        else:
            print("wmediumd is running")
...
class w_txpower(object):
    def __init__(self, staintf, sta_txpower):
        ...
        """
        self.staintf = staintf
        self.sta_txpower = int(sta_txpower)
...
class w_server(object):
    ...
    def connect(cls, uds_address=w_cst.SOCKET_PATH):
        ...
        print("starting to connect the wmeidumd sock")
        if cls.connected:
            print("Already connected to wmediumd server.")
            return  
        # 或者抛出异常
        ...
        try:
            ...
            #             # info(f"Retrying connection ({attempt + 1}/{max_retries})...")
            #             print(f"Retrying connection ({attempt + 1}/{max_retries})...")
            #             time.sleep(1)
            #         else:
            #             raise WmediumdException(f"Failed to connect after {max_retries} attempts: {str(e)}")
            #     except Exception as e:
            #         raise WmediumdException(f"Unexpected error: {str(e)}")
                
        except FileNotFoundError:
            raise WmediumdException(f"Socket file {uds_address} does not exist. Is wmediumd running?")
        except PermissionError:
            raise WmediumdException(f"Permission denied to access {uds_address}")
        except ConnectionRefusedError:
            raise WmediumdException(f"Connection refused. Check if wmediumd is accepting connections.")
    ...
    @classmethod
    def __parse_response(cls, expected_type, resp_struct):
        """Parse response from wmediumd server"""
        try:
            # Print debug info about expected size
            print(f"Expected response size: {resp_struct.size} bytes")
            
            # Receive data with timeout
            cls.sock.settimeout(5.0)  # Set 5 second timeout
            ...
            while len(recvd_data) < resp_struct.size:
                if time.time() - start_time > timeout:
                    raise WmediumdException("Timeout while receiving data")
                    
                try:
                    chunk = cls.sock.recv(min(resp_struct.size - len(recvd_data), 1024))
                    if not chunk:
                        if len(recvd_data) == 0:
                            # No data received at all - server might be down
                            raise WmediumdException("Server closed connection without sending data")
                        else:
                            # Partial data received - try to reconnect
                            print("Connection lost. Attempting to reconnect...")
                            cls.reconnect()
                            continue
                            
                    recvd_data += chunk
                    print(f"Received chunk of {len(chunk)} bytes. Total: {len(recvd_data)}/{resp_struct.size}")
                    
                except socket.timeout:
                    print("Socket timeout, retrying...")
                    continue
                    
            # Print debug info about received data
            print(f"Received complete data: {len(recvd_data)} bytes")
            
            # Verify message type
            if len(recvd_data) >= 1:
                recvd_type = cls.__base_struct.unpack(recvd_data[0:1])[0]
                if recvd_type != expected_type:
                    raise WmediumdException(
                        f"Received response of unknown type {recvd_type}, expected {expected_type}")
            else:
                raise WmediumdException("Received data too short to contain message type")
            
            # Unpack the complete response
            return resp_struct.unpack(recvd_data)
            
        except struct.error as e:
            print(f"Structure error: Expected {resp_struct.size} bytes, got {len(recvd_data)} bytes")
            print(f"Received data (hex): {recvd_data.hex()}")
            raise WmediumdException(f"Data structure mismatch: {str(e)}")
        except socket.error as e:
            print(f"Socket error: {str(e)}")
            raise WmediumdException(f"Socket error while receiving data: {str(e)}")
        finally:
            cls.sock.settimeout(None)  # Reset timeout

    @classmethod
    def reconnect(cls):
        """Attempt to reconnect to the wmediumd server"""
        try:
            cls.sock.close()
        except:
            pass
