...
class Mac80211Hwsim(object):
    ...
    def __init__(self, on_the_fly=False, Node=None):
        ...
        if on_the_fly:
            self.configNodeOnTheFly(Node)
        else:
            self.start(Node)

    def get_wlan_list(self, phys):
        # if 'docker' in params:
        #     wlan_list = []
    ...
    def configPhys(self, node):
        phys = self.get_intf_list(self.get_hwsim_list())  # gets virtual and phy interfaces
        wlan_list = self.get_wlan_list(phys)  # gets wlan list
        
        self.assign_iface(node, phys, wlan_list, (len(phys)-1))
    ...
    def configNodeOnTheFly(self, nodes):
        
        self.create_hwsim(len(Mac80211Hwsim.hwsim_ids))
        self.configPhys(nodes)
    ...
    def create_hwsim(self, n):
        self.get_phys()
        p = Popen(["./mac80211_hwsim_mgmt-master/hwsim_mgmt/hwsim_mgmt", "-c", "-n", self.prefix + ("%02d" % n)],
        ...
        try:
            #for node in range(len(nodes)):
                ...
                if False:
                    ...
                    node.cmd('ip link set {} name wlan{}'.format(wlan_list[0],node.name), stdout=True, stderr=True, stdin=False, tty=True)
                wlan_list.pop(0)
                phys.pop(0)
        except:
            exception("Warning:")
